...
GO_LDFLAGS="-s -w \
  -X arhat.dev/abbot/pkg/version.branch=${GIT_BRANCH} \
  -X arhat.dev/abbot/pkg/version.commit=${GIT_COMMIT} \
  -X arhat.dev/abbot/pkg/version.tag=${GIT_TAG} \
  -X arhat.dev/abbot/pkg/version.arch=${ARCH} \
  -X arhat.dev/abbot/pkg/version.goCompilerPlatform=$(go version | cut -d\  -f4)"
